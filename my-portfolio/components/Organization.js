...
const ExperienceTemplate = ({ title = 'Organizations', items = [], iconMap = {}, isDarkModeForced = null }) => {
    ...
    <section id="experience" className={`py-24 text-center relative overflow-hidden transition-colors duration-500 ease-in-out ${isDarkMode ? 'bg-gradient-to-br from-gray-900 to-black' : 'bg-gradient-to-br from-gray-50 to-gray-100'}`}>
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        <motion.h2
          ...
          className={`text-4xl font-extrabold mb-8 inline-block relative ${isDarkMode ? 'text-purple-400' : 'text-gray-900'}`}
        >
          {title}
          <motion.span
            className={`absolute -bottom-2 left-0 w-full h-1 ${isDarkMode ? 'bg-purple-500' : 'bg-purple-600'}`}
            initial={{ scaleX: 0 }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {experienceItems.map((exp, index) => (
            <OrganizationCard key={exp.title} {...exp} index={index} iconMap={iconMap} />
          ))}
        </div>
      </div>
      <button
        onClick={() => setIsDarkMode(!isDarkMode)}
        className={`fixed bottom-4 right-4 p-3 rounded-full text-white transition-all duration-500 ease-in-out transform hover:scale-110 hover:rotate-12 ${isDarkMode ? 'bg-purple-600 hover:bg-purple-700' : 'bg-blue-600 hover:bg-blue-700'}`}
      >
        {isDarkMode ? '🌙' : '☀️'}
      </button>
    </section>
